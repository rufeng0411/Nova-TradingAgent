...
def create_news_analyst(llm, data_collector=None):
    ...
    async def news_analyst_node(state):
        ...
        if pool is not None:
            ...
            insider_data = pool.get("insider_transactions", "无数据")
        else:
        ...
        disclosure_text = "未启用公告结构化数据。"
        if os.getenv("TA_MACRO_SYNC_ENABLED", "0").strip().lower() in ("1", "true", "yes", "on"):
            from tradingagents.agents.utils.macro_data_tools import query_disclosure

            try:
                disclosure_text = await asyncio.to_thread(
                    query_disclosure.invoke,
                    {
                        "symbol": ticker,
                ...
                disclosure_text = f"公告结构化数据调用失败：{exc}"
