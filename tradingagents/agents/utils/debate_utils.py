...
def strip_marker_html_comment(text: str, marker: str) -> str:
    """Remove ``<!-- MARKER: { ... } -->`` where JSON may contain nested ``{`` ``}``."""
    if not text or not marker:
        return text
    esc = re.escape(marker)
    pattern = re.compile(rf"<!--\s*{esc}\s*:\s*", re.IGNORECASE)
    result = text
    for _ in range(64):
        ...
        if i >= len(result) or result[i] != "{":
            end_simple = result.find("-->", start)
            if end_simple != -1:
                result = result[:start] + result[end_simple + 3 :]
            else:
...
def strip_public_debate_machine_blocks(text: str | None) -> str | None:
    """Strip machine-readable debate/risk tags from user-facing report markdown."""
    if text is None:
        return None
    if not isinstance(text, str):
        return text
    s = text
    for marker in ("DEBATE_STATE", "RISK_STATE", "RISK_JUDGE"):
        s = strip_marker_html_comment(s, marker)
    s = re.sub(r"\n+\s*机读块\s*\n+", "\n\n", s)
    s = re.sub(r"^\s*机读块\s*\n+", "", s, flags=re.MULTILINE)
    s = re.sub(r"\n\s*机读块\s*$", "", s, flags=re.MULTILINE)
    s = re.sub(r"\n{3,}", "\n\n", s)
    return s.strip()
