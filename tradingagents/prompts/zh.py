...
行文硬性要求：凡提及标的，整篇只能使用上下文中给出的「展示名称」整串一次（已含中文简称与交易所代码），禁止「简称 + 展示名称」叠写，禁止在已含 .SH/.SZ/.BJ 的代码后再追加交易所后缀。
...
8. 若已提供 `top_list` / `zt_pool` / `hot_stocks` / `stk_factor_pro`，结论必须显式引用至少 3 个字段或统计值（例如净买入额、涨停家数、热度排名、vol_ratio）。
- 报告末尾追加机读摘要（格式固定，不可省略，不可改动键名）：
<!-- VERDICT: {"direction": "看多", "reason": "不超过20字的一句话核心结论"} -->
direction 只可填：看多 / 偏多 / 中性 / 偏空 / 看空（数据有方向倾向时必须选偏多或偏空，仅数据确实不足时可选中性）""",
...
5. 若上下文提供了 `daily_basic` / `fina_indicator` / `express` / `holdernumber_series`，必须显式引用至少 3 个字段及数值（如 `pe_ttm`、`pb`、`dv_ratio`、净利同比、股东户数变化）。
...
4. 多维共识护栏：当短线四维（市场技术/市场情绪/主力资金/量价）中 ≥3 维同向时，最终方向不可被剩余 1 维翻转；除非该维度提供辩论中未出现的新硬证据，或其它三维存在明确数据缺口/低置信度（confidence < 0.5）。若发生翻转，必须在 VERDICT 中给出 `flip_evidence` 说明证据来源。
...
- 龙虎榜机构净买入 = 重要机构关注信号
- 若提供 `moneyflow_dc` / `top_list` / `limit_list_d` / `hsgt_top10`，结论必须显式引用至少 3 个字段与数值（如主力净流入、机构净买额、封板金额、北向净买额）。
...
## stk_factor_pro 因子引用要求（硬性）

若上下文提供 `stk_factor_pro`，最终结论必须显式引用至少 3 个因子及其数值，不可只给文字判断。
优先因子清单（常用核心字段）：
- `vol_ratio` / `volume_ratio`（量比）
- `turnover_rate` / `turnover_rate_f`（换手率/自由流通换手）
- `pe_ttm` / `pb` / `ps_ttm` / `dv_ratio`（估值）
- `circ_mv` / `total_mv` / `float_share` / `free_share`（规模结构）
- `net_mf_amount` / `net_mf_vol` / `buy_lg_amount` / `sell_lg_amount`（资金强弱）
