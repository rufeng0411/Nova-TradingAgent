...
AKSHARE_HEAVY_CALL_LOCK = _AkshareLock(total=5, scheduled_max=3)
AKSHARE_LIGHT_CALL_LOCK = _AkshareLock(total=10, scheduled_max=6)
# Backward-compatible alias for existing call sites.
AKSHARE_CALL_LOCK = AKSHARE_HEAVY_CALL_LOCK
...
class CnAkshareProvider(BaseMarketDataProvider):
    ...
    def _locked(self, func, *args, **kwargs):
        ...
        lock = AKSHARE_LIGHT_CALL_LOCK if getattr(func, "__name__", "") in light_names else AKSHARE_HEAVY_CALL_LOCK
        with lock:
    ...
    def fetch_daily_bar_df(
        ...
        del adjust  # current implementation already uses qfq in _fetch_hist_df
        df = self._fetch_hist_df(symbol, start_date, end_date)
        if df is None or df.empty:
            return pd.DataFrame(
                columns=["Date", "Open", "High", "Low", "Close", "Volume", "Amount", "AdjFactor"]
            )
        out = df.copy()
        out["Amount"] = None
        out["AdjFactor"] = None
        return out[["Date", "Open", "High", "Low", "Close", "Volume", "Amount", "AdjFactor"]]
    ...
    def _fetch_quotes_sina(self, code_to_original: dict[str, str]) -> str:
        ...
        for code, original in code_to_original.items():
            original_upper = (original or "").upper()
            if original_upper.endswith(".SH") or original_upper.startswith("SH"):
                prefix = "sh"
            elif original_upper.endswith(".SZ") or original_upper.startswith("SZ"):
                prefix = "sz"
            elif original_upper.endswith(".BJ") or original_upper.startswith(("BJ", "NQ")):
                prefix = "bj"
            else:
        ...
        try:
            ...
            with AKSHARE_CALL_LOCK:
                if hasattr(ak, "stock_sector_fund_flow_rank"):
                    ...
                    df = ak.stock_industry_summary_ths()
            if df is None or df.empty:
    ...
    def get_lhb_detail(self, symbol: str, date: str) -> str:
        ...
        try:
            ...
            if df is not None and not df.empty:
                filtered = None
                for key in ("代码", "股票代码", "ts_code"):
                    if key in df.columns:
                        filtered = df[df[key].astype(str).str.contains(code, na=False)]
                        break
    ...
    def get_hot_stocks_xq(self) -> str:
        ...
        try:
            ak = self._ak()
            with AKSHARE_CALL_LOCK:
                if hasattr(ak, "stock_hot_rank_xq"):
                    df = ak.stock_hot_rank_xq()
                elif hasattr(ak, "stock_hot_follow_em"):
