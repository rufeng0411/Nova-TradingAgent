...
与 `.env.example` 顶部 **Quick start** 对齐。不写架构全书，不列 Tushare 全量接口。
...
| 变量 | 默认 | 说明 |
| --- | --- | --- |
| `TA_TUSHARE_L2_ENABLED` | `0` | **opt-in**。需要 Tushare L2 权限；无权限返回空数据，分析继续 |
| `TA_FAST_ANALYSIS_ENABLED` | `0` | 快速分析 |
| `TA_USER_TASK_QUEUE_ENABLED` | `1` | 任务中心 |
| `TA_QLIB_EVAL_ENABLED` 等 | `0` | Qlib 不在 Docker 镜像内 |
