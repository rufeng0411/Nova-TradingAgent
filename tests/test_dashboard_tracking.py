...
def test_fetch_live_quotes_returns_empty_when_route_to_vendor_fails(monkeypatch):
    ...
    # Both rt_k path and the legacy realtime_quotes vendor must be stubbed,
    # otherwise the real Tushare/AkShare chain may answer in the test env.
    monkeypatch.setattr(
        "api.services.rt_quote_service.get_rt_daily_bulk",
        lambda symbols: ({}, list(symbols), 0),
    )
...
def test_fetch_live_quotes_returns_parsed_quotes(monkeypatch):
    ...
    # 让 rt_k 路径返回空，强制走 fallback realtime quotes 路径
    monkeypatch.setattr(
        "api.services.rt_quote_service.get_rt_daily_bulk",
        lambda symbols: ({}, list(symbols), 0),
    )
