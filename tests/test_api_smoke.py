...
def _auth(client: TestClient) -> str:
    ...
    token, _ = register_user_via_api(client, prefix="apitest")
    return token
...
class TestWecomRuntimeConfig:
    ...
    def test_wecom_warmup_uses_stored_webhook_when_input_missing(self):
        ...
        assert "Nova-TradingAgent Webhook Warmup" in mock_send.call_args.args[0]
...
class TestWatchlistAddEndpoint:
    ...
    def test_batch_add_accepts_bare_a_share_code_when_stock_map_empty(self):
        ...
        assert body["results"][0]["symbol"] == "600330.SH"

    def test_batch_add_accepts_common_stock_name_alias_when_stock_map_empty(self):
        with patch("api.main._load_cn_stock_map", return_value={}), \
             patch("api.main._get_reverse_stock_map", return_value={}):
            r = self.client.post("/v1/watchlist", headers=self.headers, json={
                "text": "鼎盛新材",
        ...
        assert body["summary"] == {"total": 1, "added": 1, "duplicate": 0, "failed": 0}
        assert body["results"][0]["status"] == "added"
        assert body["results"][0]["symbol"] == "603876.SH"
        assert body["results"][0]["name"] == "鼎胜新材"
...
class TestKlineAndChartInsight:
    def test_kline_invalid_period_422(self):
        client = _get_client()
        r = client.get("/v1/market/kline", params={"symbol": "000001.SH", "period": "2d"})
        assert r.status_code == 422

    def test_chart_insight_requires_auth(self):
        client = _get_client()
        r = client.post("/v1/market/chart/insight", json={"symbol": "000001.SH"})
