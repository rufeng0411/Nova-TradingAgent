...
import { Save, Key, Database, Loader2, Trash2, Link2, Copy, Plus, CheckCircle2, Mail, Flame, Webhook, Monitor, Sun, Moon, Palette } from 'lucide-react'
...
function applyThemeDom(mode: ThemeMode) {
...
export default function Settings() {
    ...
    const [appearanceTheme, setAppearanceTheme] = useState<ThemeMode>(() => {
        const raw = localStorage.getItem('ta-theme') || 'system'
    ...
    const handleAppearanceTheme = (m: ThemeMode) => {
        localStorage.setItem('ta-theme', m)
        applyThemeDom(m)
    ...
    useEffect(() => {
        const onSync = () => {
            const raw = localStorage.getItem('ta-theme') || 'system'
            const m: ThemeMode = raw === 'light' || raw === 'dark' || raw === 'system' ? raw : 'system'
            setAppearanceTheme(m)
        }
    ...
    useEffect(() => {
        try {
            const key = perUserLocalStorageKey('tradingagents-settings')
            let stored = localStorage.getItem(key)
            if (!stored) {
                const legacy = localStorage.getItem('tradingagents-settings')
                if (legacy) {
    ...
    const persistLocalSettings = () => {
        const sk = perUserLocalStorageKey('tradingagents-settings')
        const pk = perUserLocalStorageKey('ta-custom-prompt')
        localStorage.setItem(sk, JSON.stringify({
            defaultAnalysts,
            customPrompt,
        }))
        localStorage.setItem(pk, customPrompt)
    }
        ...
        <div className="space-y-6">
            ...
            <div className="card space-y-4">
                <div className="flex items-center gap-2">
                    ...
                    <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100">外观</h2>
                </div>
                <p className="text-sm text-slate-500 dark:text-slate-400">
                    界面皮肤与浅色 / 深色模式（与顶栏头像菜单中的「主题模式」同步）。
                </p>
                <div>
                    <div className="block text-sm font-medium text-slate-600 dark:text-slate-400 mb-2">皮肤</div>
                    <div className="flex flex-wrap gap-4">
                        <label className="inline-flex items-center gap-2 cursor-pointer text-sm text-slate-700 dark:text-slate-300">
                            <input
                                type="radio"
                                name="ta-skin-settings"
                                checked={skin === 'default'}
                                onChange={() => setSkin('default')}
                            />
                            默认
                        </label>
                        <label className="inline-flex items-center gap-2 cursor-pointer text-sm text-slate-700 dark:text-slate-300">
                        ...
                        <label className="inline-flex items-center gap-2 cursor-pointer text-sm text-slate-700 dark:text-slate-300">
                            <input
                                type="radio"
                                name="ta-skin-settings"
                                checked={skin === 'graphite'}
                                onChange={() => setSkin('graphite')}
                            />
                            石墨
                        </label>
                    </div>
                ...
                <div>
                    <div className="block text-sm font-medium text-slate-600 dark:text-slate-400 mb-2">主题模式</div>
                    <div className="flex flex-wrap gap-2">
                        <button
                            type="button"
                            onClick={() => handleAppearanceTheme('system')}
                            className={`inline-flex items-center gap-2 rounded-xl border px-3 py-2 text-sm transition-colors ${
                                appearanceTheme === 'system'
                                    ? 'border-blue-500 bg-blue-50 text-blue-700 dark:bg-blue-500/15 dark:text-blue-400'
                                    : 'border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800/50'
                            }`}
                        >
                        ...
                        <button
                            type="button"
                            onClick={() => handleAppearanceTheme('light')}
                            className={`inline-flex items-center gap-2 rounded-xl border px-3 py-2 text-sm transition-colors ${
                        ...
                        <button
                            type="button"
                            onClick={() => handleAppearanceTheme('dark')}
                            className={`inline-flex items-center gap-2 rounded-xl border px-3 py-2 text-sm transition-colors ${
