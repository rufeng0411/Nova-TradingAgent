...
export default function UsersTrendReport() {
    ...
    const load = useCallback(async () => {
        ...
        setErr(null)
        try {
            const d = await api.adminReportsUsersTrend(toApiRange(range, grain))
    ...
    return (
        <AdminPage title="用户趋势" subtitle="新增与活跃（活跃为成功请求的去重用户，运营口径）">
            <AdminFilterBar
                range={range}
                onRangeChange={setRange}
                onRefresh={() => void load()}
                onExportCsv={() => void downloadCsv('users-trend', range, grain)}
                grain={grain}
                ...
                {!loading && chartData.length === 0 ? (
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                        <LineChart data={chartData}>
                            ...
                            <Tooltip />
                            <Legend />
                            <Line type="monotone" dataKey="new_users" name="新增用户" stroke="#2563eb" dot={false} />
                            <Line type="monotone" dataKey="active_users" name="活跃用户" stroke="#10b981" dot={false} />
                        </LineChart>
                    </ResponsiveContainer>
                )}
            </div>
