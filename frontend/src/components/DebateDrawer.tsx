...
export default function DebateDrawer({ debate, onClose }: DebateDrawerProps) {
    ...
    return (
        <>
            ...
            {/* Drawer */}
            <div className="fixed top-0 right-0 h-full w-1/2 max-w-[720px] min-w-[400px] bg-white dark:bg-slate-900 border-l border-slate-200 dark:border-slate-700 shadow-2xl z-50 flex flex-col animate-in slide-in-from-right duration-300">
                {/* Header */}
                <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 dark:border-slate-800">
                    <div className="flex items-center gap-3">
                        <span className="text-lg">{meta?.emoji}</span>
                        <h2 className="text-lg font-bold text-slate-900 dark:text-white tracking-tight">{meta?.title}</h2>
                    </div>
                    <button
                        onClick={onClose}
                ...
                {/* Participant pills */}
                <div className="flex items-center gap-2 px-5 py-3 border-b border-slate-100 dark:border-slate-800/50">
                    {participants.map(p => (
