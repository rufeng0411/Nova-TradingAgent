...
def _report_display_label_for_email(report: "ReportDB", stock_name: str = "") -> str:
    """统一邮件标题与摘要行的标的展示（名称 + 规范代码）。"""
    from api.services import symbol_service

    sym_raw = (report.symbol or "").strip()
    sym_key = sym_raw.upper()
    nm = (stock_name or "").strip()
    if not nm:
        nm = symbol_service.display_name_for_symbol(sym_key)
    return symbol_service.format_display_label(nm if nm.upper() != sym_key else None, sym_raw)
...
def render_report_html(report: "ReportDB", frontend_url: str = "", stock_name: str = "") -> str:
    ...
    display_label = _escape(_report_display_label_for_email(report, stock_name))
    ...
    parts.append(
        ...
        f'<td><p style="margin:0;font-size:22px;font-weight:700;color:#ffffff;letter-spacing:-0.3px;">Nova-TradingAgent 投研报告</p>'
        f'<p style="margin:6px 0 0;font-size:14px;color:#94a3b8;">{display_label} &middot; {trade_date}</p></td>'
    ...
    parts.append(
        ...
        f'<p style="margin:0;font-size:11px;text-transform:uppercase;letter-spacing:1px;color:#64748b;">模型归纳符号</p>'
    ...
    if target_price is not None or stop_loss is not None:
        ...
        if target_price is not None:
            parts.append(
                ...
                f'<p style="margin:0;font-size:11px;color:#991b1b;text-transform:uppercase;letter-spacing:0.5px;">&#127919; 偏多参考峰值</p>'
        ...
        if stop_loss is not None:
            parts.append(
                ...
                f'<p style="margin:0;font-size:11px;color:#166534;text-transform:uppercase;letter-spacing:0.5px;">&#128737; 偏空参考风控</p>'
    ...
    if ftd:
        ...
        parts.append(
            ...
            '<p style="margin:0 0 12px;font-size:15px;font-weight:700;color:#0f172a;">&#128221; 沙盘综合研判结论</p>'
    ...
    parts.append(
        ...
        '<p style="margin:0;font-size:12px;color:#94a3b8;line-height:1.6;">本报告由 Nova-TradingAgent 多智能体系统自动生成，仅供参考，不构成投资建议。</p>'
        '<p style="margin:10px 0 0;font-size:12px;color:#94a3b8;">'
        'Nova-TradingAgent &mdash; 14 名 AI Agent 协作分析的 A 股智能投研系统</p>'
...
def send_report_email(user: "UserDB", report: "ReportDB", stock_name: str = "") -> bool:
    ...
    display_name = _report_display_label_for_email(report, stock_name)
    ...
    msg = EmailMessage()
    msg["Subject"] = f"Nova-TradingAgent 投研报告 - {display_name} ({trade_date})"
    msg["From"] = smtp_from
    msg["To"] = user.email

    # text/plain fallback
    plain = f"Nova-TradingAgent 投研报告\n{display_name} {trade_date}\n模型归纳符号: {report.decision or '-'}\n方向: {report.direction or '-'}\n置信度: {report.confidence or '-'}%{report_link}\n\n请使用支持 HTML 的邮件客户端查看完整报告。"
