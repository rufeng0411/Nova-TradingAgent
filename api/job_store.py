...
def get_job_store() -> JobStore:
    ...
    Returns RedisJobStore when REDIS_URL is set and Redis answers PING.
    Otherwise InMemoryJobStore (including when Redis is configured but
    unreachable — common in local dev without redis-server).
    """
    ...
    if redis_url:
        try:
            ...
            logger.warning(
                "REDIS_URL is set but Redis is not reachable (%s: %s); "
                "falling back to InMemoryJobStore (OK for single-worker local dev; "
                "start Redis or unset REDIS_URL to silence this).",
                type(exc).__name__,
                exc,
            )
